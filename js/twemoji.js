...
(function(){
    chrome.extension.sendMessage(true, function (response) {
        var initEmoji = function(){
            var twemojiStyle = document.createElement("style");
            twemojiStyle.id = 'twemoji-style';
            twemojiStyle.innerHTML = response.style;
            if (!!document.head && !document.getElementById('twemoji-style')) {
                document.head.appendChild(twemojiStyle);
            }
            twemoji.parse(document.body, { base: chrome.extension.getURL(''), ext: '.svg', size: 'svg', className: 'chrome-twemoji' });
        },
        disEmoji = function(){
            twemoji.parse('');
        };
        var uselist = response.uselist;
        if (response[uselist].length) {
            response[uselist].forEach(function(item){
                var ori = document.location.origin;
                var inURL = (ori.indexOf(item) > -1 && ori.indexOf(item) === ori.length - item.length);
                if (uselist === 'blacklist') {
                    return inURL ? disEmoji() : initEmoji();
                } else {
                    return inURL ? initEmoji() : disEmoji();
                }
            });
        } else {
            initEmoji();
